...
export default function TrafficChart({ result }) {
  if (!result) {
     return (
        <div className="glass-panel rounded-2xl p-6 flex items-center justify-center min-h-[300px] text-sm text-white/35 border-t-2 border-t-emerald/50">
           Run a simulation to see traffic flow changes.
        </div>
     );
  }
  ...
  const data = [
    {
      name: 'Before',
      'Vehicle Count': result.before.vehicle_count,
      'Road Capacity': result.before.road_capacity,
    },
    {
      name: 'After',
      'Vehicle Count': result.after.vehicle_count,
      'Road Capacity': result.after.road_capacity,
    }
  ];
    ...
    <div className="glass-panel rounded-2xl p-6 border-t-2 border-t-emerald/50 relative group">
      ...
      <div className="flex items-center justify-between mb-5 relative z-10">
        <h3 className="font-semibold text-white text-sm flex items-center gap-2">
          ...
          Traffic & Capacity <span className="text-white/35 font-normal text-xs uppercase tracking-widest">(Before vs After)</span>
        </h3>
      </div>
      <div className="relative z-10">
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={data} margin={{ top: 4, right: 16, left: -12, bottom: 0 }}>
            ...
            <Bar dataKey="Vehicle Count" fill="#00e5a0" radius={[4, 4, 0, 0]} />
            <Bar dataKey="Road Capacity" fill="#ffffff" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
